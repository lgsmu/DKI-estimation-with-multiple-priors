function [x,fval_total_obj]=fminlbfgs(funfcn,x_init,optim)
...
%The code is modified based on fminlbfgs.m provided
% by D.Kroon University of Twente (March 2009)
...
%   
%   [X,FVAL,EXITFLAG] = FMINLBFGS(FUN,X0,OPTIONS) 
%
%   Inputs,
...
defaultopt = struct('Display','final','HessUpdate','bfgs','GoalsExactAchieve',1,'GradConstr',true,  ...
			'TolX',1e-8,'TolFun',1e-8,'GradObj','off','MaxIter',1001,'MaxFunEvals',100*numel(x_init)-1,  ...
...
if(strcmp(optim.Display,'iter'))
    disp(' Iteration     Func-count     Grad-count      f(x)       normgrad        Step-size');
end
...
data.gradient=grad;%%%updata gradient
data.dir = -data.gradient;
data.gOld=grad;
data.fInitial = fval;%%%updata fval
data.fPrimeInitial= data.gradient'*data.dir(:);
    

gNorm = norm(data.gradient,2);  % Norm of gradient
data.initialStepLength = min(1/gNorm,5); 

% Show the current iteration
if(strcmp(optim.Display,'iter'))
        s=sprintf('%5.0f       %5.0f      %5.0f             %13.6g                  %13.6g        %13.6g ',data.iteration,data.funcCount,data.gradCount,data.fInitial,gNorm,data.initialStepLength); disp(s);
end
...
while(true)
    ...
    gNorm(iii) = norm(data.gradient,2);  % Norm of gradient
    fval_total_obj(iii) = data.fInitial;

    
    % Set exit flags 
    if(gNorm(iii) <optim.TolFun), exitflag=1; end
    ...
    if(strcmp(optim.Display(1),'i')||strcmp(optim.Display(1),'p'))
        s=sprintf('%5.0f       %5.0f      %5.0f             %13.6g                   %13.6g        %13.6g ',data.iteration,data.funcCount,data.gradCount,data.fInitial,gNorm(iii),data.initialStepLength); disp(s);
    end
...
if(isfield(data,'beta')&&(data.f_beta+0.001<f_alpha_estimated)), alpha=data.beta; end
...
if((~isempty(i))&&(~isnan(data.storegx(i))))
    ...
    [data,f_alpha, grad]=gradient_function(data.xInitial(:)+alpha*data.dir(:),funfcn, data,optim);
    if(isfield(data,'beta')&&(data.f_beta+0.001<f_alpha)), 
        alpha=data.beta; 
        if((~isempty(i))&&(~isnan(data.storegx(i))))
            f_alpha=data.storefx(i); grad=data.storegx(:,i);
        else
...
%         return; 
    ...
    if(~optim.GradConstr)
        [data,f_alpha, grad,f_alpha_1,f_alpha_2,f_alpha_3]=gradient_function(data.xInitial(:)+alpha*data.dir(:),funfcn, data, optim);
        fPrime_alpha = grad'*data.dir(:);
    else
        gstep=data.initialStepLength/1e6; 
        if(gstep>optim.DiffMaxChange), gstep=optim.DiffMaxChange; end
    ...
    if ((f_alpha > data.fInitial + alpha*optim.rho*data.fPrimeInitial) || (f_alpha >= data.f_a))
        ...
        %
%            if (abs(alpha-data.a) >10*eps)%%%xinyuan
