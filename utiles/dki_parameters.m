function [fa, md,  mk] = dki_parameters(dt, mask)    
n = size(dt, 4);
if ndims(dt)~=4
...
if ~ismatrix(dt)
    ...
    dt_ = zeros([size(dt,4) sum(mask(:))], class(dt));
    for k = 1:size(dt,4);
            tmp = dt(:,:,:,k);
            dt_(k,:) = tmp(mask(:));
    end
...
dims = size(mask);
% unvec fa
fa_ = NaN([dims 1], 'double'); md_=fa_;mk_=fa_;
fa_(mask) = fa; md_(mask) = md;mk_(mask) = mk; 
fa = fa_; md = md_;  mk = mk_; 
clear fa_ md_  mk_  
